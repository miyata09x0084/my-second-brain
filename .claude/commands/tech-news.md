...
まずリポジトリルートの `.env` ファイルを Read で読み込み、以下の変数値を取得する:

- `HACKER_NEWS_TOP_STORIES_URL`
- `HACKER_NEWS_ITEM_URL_TEMPLATE`（`{id}` プレースホルダを含む）
- `TECHCRUNCH_RSS_URL`
- `REDDIT_TECH_URL`

以降の手順では取得した変数値を使用する（ハードコード禁止）。
...
```
WebFetch で ${HACKER_NEWS_TOP_STORIES_URL} を取得する。
返ってきたIDリストの先頭10件について、それぞれ ${HACKER_NEWS_ITEM_URL_TEMPLATE} の {id} を実際のIDに置換して取得する。
各記事から以下を抽出して返す:
- title: 記事タイトル
- url: 記事URL
- score: スコア
...
```
WebFetch で ${TECHCRUNCH_RSS_URL} を取得する。
...
```
WebFetch で ${REDDIT_TECH_URL} を取得する。
